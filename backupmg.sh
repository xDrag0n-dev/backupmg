...
# Check if the script is run with privileges
if [[ $UID -ne 0 ]]; then
  echo -e "${RED}[!] This program must be run in root mode or sudo privileges${RESET}"
  exit 1
fi

# Check if the directory to save exists
if [[ $# -eq 1 ]]; then
...
if [[ $response -eq 1 ]]; then
  ...
  # Get the size of the directory or filesystem to save
  ...
  # Check if the image to load exists
